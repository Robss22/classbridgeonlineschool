...
export default function TeacherJoinLiveClassPageClient() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      // Pre-join: validate class can be started using comprehensive time validation
      try {
        ...
        if (lc && lc.scheduled_date && lc.start_time && lc.end_time) {
          ...
          // Additional check: ensure class hasn't ended
          const now = new Date();
        ...
        console.error('Error validating class time:', err);
        setError('Failed to validate class schedule');
        return;
      }
