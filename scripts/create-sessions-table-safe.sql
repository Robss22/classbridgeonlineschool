-- Create sessions table for single-device login management (Safe Version)
...
DROP TRIGGER IF EXISTS trigger_deactivate_old_sessions ON user_sessions;

-- Drop existing functions if they exist
DROP FUNCTION IF EXISTS update_session_activity() CASCADE;
DROP FUNCTION IF EXISTS deactivate_old_sessions() CASCADE;
DROP FUNCTION IF EXISTS cleanup_expired_sessions() CASCADE;

-- Drop existing view if it exists
DROP VIEW IF EXISTS session_analytics;

-- Drop existing table if it exists (this will also drop indexes and constraints)
